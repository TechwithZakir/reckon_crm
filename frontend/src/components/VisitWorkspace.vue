...
async function open(name) { try { selected.value = await api('visits.get_visit', { name }, 'GET') } catch (e) { error.value = e.message } }
...
watch(() => [props.doctype, props.docname], () => { selected.value = null; scheduling.value = false; load() }, { immediate: true })
</script>
